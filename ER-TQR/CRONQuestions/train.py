...
from qa_ER-TQR import QA_ER-TQR
from qa_datasets import QA_Dataset, QA_Dataset_ER-TQR, QA_Dataset_Baseline
...
def eval(qa_model, dataset, batch_size = 128, split='valid', k=10):
    ...
    for k in k_list:
        ...
        with open('cronResult.txt', 'w') as f:
            for i, question in enumerate(dataset.data):
                ...
                f.write(str(list(predicted)))
                f.write('\n')
                if len(set(actual_answers).intersection(set(predicted))) > 0:
                    val_to_append = 1
                    hits_at_k += 1
                    if list(set(actual_answers).intersection(set(predicted)))[0] in list(predicted):
                        rank = list(predicted).index(list(set(actual_answers).intersection(set(predicted)))[0]) + 1
                        recp_ranks.append(1.0 / rank)
                    else:
                        recp_ranks.append(0.0)
                else:
                    val_to_append = 0
...
if args.model == 'bert' or args.model == 'roberta':
    ...
    #valid_dataset = QA_Dataset_baseline(split=args.eval_split, dataset_name=args.dataset_name)
    test_dataset = QA_Dataset_Baseline(split=test, dataset_name=args.dataset_name)
elif args.model in ['ER-TQR', 'entityqr', 'cronkgqa']:  #supervised models
    qa_model = QA_ER-TQR(tkbc_model, args)
    if args.mode == 'train':
        dataset = QA_Dataset_ER-TQR(split=train_split, dataset_name=args.dataset_name, args=args)
    #valid_dataset = QA_Dataset_ER-TQR(split=args.eval_split, dataset_name=args.dataset_name, args=args)
    test_dataset = QA_Dataset_ER-TQR(split=test, dataset_name=args.dataset_name, args=args)
else:
...
print(f'used memory : {memory_after-memory_before}s')
