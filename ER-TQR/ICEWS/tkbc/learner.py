...
models = [
    'TComplEx', 'TNTComplEx', 'ER-TQR_Quaternion', 'ER-TQR_Complex'
]
parser.add_argument(
...
model = {
    ...
    'ER-TQR_Complex':ER-TQR_Complex(sizes, args.rank, cycle=args.cycle),
    'ER-TQR_Quaternion':ER-TQR_Quaternion(sizes, args.rank, cycle=args.cycle),
}[args.model]
model = model.cuda()
best_hits1 = 0
best_res_test = {}
...
for epoch in range(args.max_epochs):
    ...
    def avg_both(mr, mrrs: Dict[str, float], hits: Dict[str, torch.FloatTensor], added):
        ...
        mr = (mr['lhs'] + mr['rhs']) / 2.
        return {'MR':mr, 'MRR': m, 'hits@[1,5,10]': h, 'add':added}
    if epoch < 0 or (epoch + 1) % args.valid_freq == 0:
        if dataset.has_intervals():
            ...
            print("valid: ", epoch, valid['MR'], valid['MRR'], valid['hits@[1,5,10]'])
            print("test: ", epoch, test['MR'], test['MRR'], test['hits@[1,5,10]'])
            fw.write("valid: epoch:{}, MR:{}, MRR:{}, Hist:{}\n".format(epoch, valid['MR'], valid['MRR'], valid['hits@[1,5,10]']))
            fw.write("test: epoch:{}, MR:{}, MRR:{}, Hist:{}\n".format(epoch, test['MR'], test['MRR'], test['hits@[1,5,10]']))
        if valid['hits@[1,5,10]'][0] > best_hits1:
            torch.save({'MRR':test['MRR'], 'Hist':test['hits@[1,5,10]'], 'MR':test['MR'], 'param':model.state_dict()}, '{}/best.pth'.format(save_path, args.model, args.dataset))
            print('best')
            best_hits1 = valid['hits@[1,5,10]'][0]
            best_res_test = [test['MR'], test['MRR'], test['hits@[1,5,10]']]
