            ...
            while c_begin < self.sizes[2]:
                ...
                while b_begin < len(queries):
                    ...
                    if self.model_name in ['TNTComplEx' , 'TComplEx']:
                        ...
                        scores = q@rhs
                    elif self.model_name == 'ER-TQR_Quaternion':
...
class ER-TQR_Quaternion(TKBCModel):
    def __init__(
            self, sizes: Tuple[int, int, int, int], rank: int,
            init_size: float = 1e-3, cycle=365
    ):
        super(ER-TQR_Quaternion, self).__init__()
        self.model_name = "ER-TQR_Quaternion"
        self.cycle = cycle
        self.sizes = sizes
        self.rank = rank
...
class ER-TQR_Complex(TKBCModel):
    def __init__(
            self, sizes: Tuple[int, int, int, int], rank: int,
            init_size: float = 1e-3, cycle=365
    ):
        super(ER-TQR_Complex, self).__init__()
        self.cycle = cycle
        self.model_name = "ER-TQR_Complex"
        self.sizes = sizes
        self.rank = rank
        self.pi = 3.14159265358979323846
